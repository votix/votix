console.log("starrating29.js is running!");
document.addEventListener('DOMContentLoaded', function() {
    StarRatingGenerator = typeof StarRatingGenerator == "undefined" ? 0 : StarRatingGenerator + 1, function(elementIndex) {
        ...
        if (elementIndex === 0 && !document["querySelectorAll"]('script[src="https://cdn.jsdelivr.net/gh/votix/votix/starrating31.js"]')) {
        ...
        function initializeStarRatingElement(ratingElementIndex) {
            console.log("Вызвана initializeStarRatingElement с индексом:", ratingElementIndex); // Проверяем, вызывается ли функция и с каким индексом
            const ratingElements = document["querySelectorAll"](".kozyr-SRS-average");
            console.log("Результат querySelectorAll('.kozyr-SRS-average'):", ratingElements); // Выводим NodeList, полученный querySelectorAll
            console.log("Длина NodeList:", ratingElements.length); // Выводим длину NodeList
            let ratingElement = ratingElements[ratingElementIndex];
            console.log("ratingElement после индексации:", ratingElement); // Выводим найденный ratingElement или undefined
            ...
            let ratingIdentifierUrl = location["host"].replace("www.", "").replace(/\./g, "_")["replace"](/\//g, "__"); // Формируем идентификатор URL для Firebase
        ...
        function initialize(ratingInitIndex) {
            console.log("Вызвана initialize с индексом:", ratingInitIndex); // Debug log for initialize

            function checkFirebaseReady() {
                console.log("Проверка готовности Firebase..."); // Debug log for Firebase check
                if (typeof firebase == "object" && typeof firebase["database"] == "function" && typeof firebase["initializeApp"] == "function") {
                    console.log("Firebase готова, инициализация элемента рейтинга."); // Debug log if Firebase is ready
                    initializeStarRatingElement(ratingInitIndex);
                } else {
                    console.log("Firebase еще не готова, повторная проверка через 50мс."); // Debug log if Firebase not ready
                    setTimeout(checkFirebaseReady, 50); // Re-check after 50ms
                }
            }

            checkFirebaseReady(); // Start the Firebase readiness check
        }
